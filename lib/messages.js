...
Rooms.prototype.setIOBasedOnTransporter = function (options) {
    ...
    switch (options.transporter.type) {
        case 'socket.io':
            ...
            transporter.listen(options.transporter.server);
            break;
    }

    transporter.on(messagetype.CONNECTION, function () {
...
Rooms.prototype.addSocketMapping = function () {
    Object.keys(messagetype).forEach(function (key) {
        transporter.onSelectedSocket(messagetype[key], function (data) {
...
Rooms.prototype[messagetype.JOIN_ROOM] = function (data) {
    ...
    transporter.set(data.roomName, function () {
...
Rooms.prototype[messagetype.REGISTER] = function (data) {
    if (!basket[data.roomName][states.USERS].hasOwnProperty(data.userId)) {
        basket[data.roomName][states.USERS][data.userId] = transporter.id();
    }
    ...
    if (basket[data.roomName][states.SUBSCRIPTIONS].hasOwnProperty('RoomInfoVO')) {
        ...
        for (id in users) {
            if (data.userId !== id) {
                transporter.send(users[id], messagetype.REQUEST_NUM_OF_USERS, retData);
        ...
        if (basket[room][states.SUBSCRIPTIONS].hasOwnProperty(data.name)) {
            ...
            for (id in users) {
                if (id !== data.userId) {
                    ...
                    transporter.send(users[id], messagetype.STATE_CHANGE, dataBack);
                }
            }
        }
    });
...
Rooms.prototype[messagetype.GET_STATE] = function (data) {
    ...
    if (size > 0) {
        ...
        transporter.send(to, messagetype.GET_STATE,object);
    } else {
...
Rooms.prototype[messagetype.REQUEST_NUM_OF_USERS] = function (data) {
    ...
    transporter.send(to, messagetype.REQUEST_NUM_OF_USERS, retData);
}

Rooms.prototype[messagetype.MESSAGE] = function (data) {
    basketutil.log("Client data: " + data);
    transporter.get('room', function (err, room) {
        transporter.sendToAll(room, data);
    });
}
...
Rooms.prototype[messagetype.DBCONNECTOR] = function (data) {
    ...
    if (roomdb.hasOwnProperty(data.methodName)) {
        ...
        roomdb[data.methodName](data, function (data, vo) {
            ...
            transporter.send(users[data.userId], messagetype.DBCONNECTOR, retVal);
        });
...
Rooms.prototype[messagetype.DISCONNECT] = function () {
    ...
    disconnectedId = transporter.findDisconnectedId(basket);
    console.log(disconnectedId);
    retObject = basketutil.cleanRoom(disconnectedId, basket, states);
    ...
    if (basketutil.checkIfRoomHasSubscriptions(retObject.room, basket, states, 'RoomInfoVO')) {
        ...
        for (id in users) {
            transporter.send(users[id], messagetype.REQUEST_NUM_OF_USERS, data);
